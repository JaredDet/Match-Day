...
def test_cancels_card_and_decrements_counter():
    ...
    use_case = CancelCardUseCase(match_repository, card_repository)
...
def test_rejects_cancelling_card_twice():
    ...
    with pytest.raises(type(MatchErrors.CardAlreadyCancelled)):
        use_case.execute(match_id=match.id, card_id=card.id)
...
def test_rejects_cancelling_card_when_match_is_not_live():
