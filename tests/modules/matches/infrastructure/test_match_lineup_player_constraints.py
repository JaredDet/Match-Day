...
def test_rejects_repeated_player_in_match_squad():
    match = MatchMother.create(persist_teams=True)
    match.save()
    player = Player.objects.create(team=match.home_team, name="Jugador")
    match.add_squad_player(player=player, shirt_number=8).save()

    with pytest.raises(IntegrityError):
        match.add_squad_player(player=player, shirt_number=10).save()
...
def test_rejects_repeated_shirt_number_for_same_team():
    ...
    match.add_squad_player(player=first, shirt_number=8).save()

    with pytest.raises(IntegrityError):
        match.add_squad_player(player=second, shirt_number=8).save()
...
def test_rejects_more_than_one_captain_for_same_team():
    ...
    match.add_squad_player(player=first, shirt_number=8, is_captain=True).save()

    with pytest.raises(IntegrityError):
        match.add_squad_player(player=second, shirt_number=10, is_captain=True).save()
